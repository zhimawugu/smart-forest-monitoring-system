...
public class LocationValidator {
    ...
    public static String validateCoordinates(double latitude, double longitude) {
        ...
        if (latitude < LATITUDE_MIN || latitude > LATITUDE_MAX) {
            return String.format("Invalid latitude: %.6f (must be between %.2f and %.2f)", latitude, LATITUDE_MIN, LATITUDE_MAX);
        }

        // Check longitude range
        if (longitude < LONGITUDE_MIN || longitude > LONGITUDE_MAX) {
            return String.format("Invalid longitude: %.6f (must be between %.2f and %.2f)", longitude, LONGITUDE_MIN, LONGITUDE_MAX);
        }
